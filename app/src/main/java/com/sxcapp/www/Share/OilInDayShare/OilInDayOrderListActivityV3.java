...
public class OilInDayOrderListActivityV3 extends BaseActivity implements XListView.IXListViewListener {
    ...
    private void loadData() {
        pageIndex = 0;
        Observable<CodeResultV3<OrderListInfoBeanV3>> ob = service.getOilInDayOrderListV3(SharedData.getInstance().getString(SharedData._user_id), pageIndex);
        ob.compose(compose(this.<CodeResultV3<OrderListInfoBeanV3>>bindToLifecycle())).subscribe(new CodeObserverV3<OrderListInfoBeanV3>(this) {
            ...
            protected void onHandleSuccess(final OrderListInfoBeanV3 infoBeanV3) {
                if (infoBeanV3.getList().size() > 0) {
                    adapter = new OrderListAdapterV3(OilInDayOrderListActivityV3.this, infoBeanV3.getList());
                        ...
                        public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                            if (infoBeanV3.getList().get(position - 1).getOrder_state() == 3) {
                                ...
                                if (infoBeanV3.getList().get(position - 1).getOrder_state() == 0) {
                                    Intent intent = new Intent(OilInDayOrderListActivityV3.this, OilExistInDayOrderActivityV3.class);
                                    intent.putExtra("order_no", infoBeanV3.getList().get(position - 1).getOrder_no());
                                    startActivity(intent);
                                } else if (infoBeanV3.getList().get(position - 1).getOrder_state() == 1) {
                                    Intent intent = new Intent(OilInDayOrderListActivityV3.this, OilInDayEndActivityV3.class);
                                    intent.putExtra("order_no", infoBeanV3.getList().get(position - 1).getOrder_no());
                                    startActivity(intent);
                                } else if (infoBeanV3.getList().get(position - 1).getOrder_state() == 2 || infoBeanV3.getList().get(position - 1).getOrder_state() == 4) {
                                    Intent intent = new Intent(OilInDayOrderListActivityV3.this, OilInDayOffOrderDetailActivityV3.class);
                                    intent.putExtra("order_no", infoBeanV3.getList().get(position - 1).getOrder_no());
