...
public class ElecShortOrderListActivityV3 extends BaseActivity implements XListView.IXListViewListener {
    ...
    private void loadData() {
        pageIndex = 0;
        Observable<CodeResultV3<OrderListInfoBeanV3>> ob = service.getElecShortOrderListV3(SharedData.getInstance().getString(SharedData._user_id), pageIndex);
        ob.compose(compose(this.<CodeResultV3<OrderListInfoBeanV3>>bindToLifecycle())).subscribe(new CodeObserverV3<OrderListInfoBeanV3>(this) {
            ...
            protected void onHandleSuccess(final OrderListInfoBeanV3 infoBeanV3) {
                if (infoBeanV3.getList().size() > 0) {
                    adapter = new OrderListAdapterV3(ElecShortOrderListActivityV3.this, infoBeanV3.getList());
                        ...
                        public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                            if (infoBeanV3.getList().get(position - 1).getOrder_state() == 3) {
                                ...
                                if (infoBeanV3.getList().get(position - 1).getOrder_state() == 6) {
                                    Intent intent = new Intent(ElecShortOrderListActivityV3.this, PayActivityV3.class);
                                    ...
                                    Intent intent = new Intent(ElecShortOrderListActivityV3.this, ElecShortOffOrderDetailActivityV3.class);
                                    intent.putExtra("order_no", infoBeanV3.getList().get(position - 1).getOrder_no());
                                    intent.putExtra("order_state", infoBeanV3.getList().get(position - 1).getOrder_state());
                                    startActivity(intent);
                                } else {
                                    Intent intent = new Intent(ElecShortOrderListActivityV3.this, BeginUseCarActivityV3.class);
                                    intent.putExtra("order_no", infoBeanV3.getList().get(position - 1).getOrder_no());
                                    intent.putExtra("order_state", infoBeanV3.getList().get(position - 1).getOrder_state() + "");
                                    startActivity(intent);
                                }
