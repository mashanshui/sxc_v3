...
public class LuckyActivityChifeng extends BaseActivity implements View.OnClickListener {
    ...
    private SparseArray<String> couponId_map = new SparseArray<>();

    @Override
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_lucky_chifeng);
        ...
        ob.compose(compose(this.<Result<CouponRemarkBean>>bindToLifecycle())).subscribe(new BaseObserver<CouponRemarkBean>(this) {
            ...
            protected void onHandleSuccess(CouponRemarkBean couponRemarkBean) {
                for (int i = 0; i < couponRemarkBean.getRemark().size(); i++) {
                    TextView tv = new TextView(LuckyActivityChifeng.this);
            ...
            for (int i = 0; i < bean.getCouponList().size(); i++) {
                switch (bean.getCouponList().get(i).getCut_cost()) {
                    case 5:
                        ...
                        break;
                    case 10:
                        map.put(10, bean.getCouponList().get(i).getSurplus_num());
                        couponId_map.put(4, bean.getCouponList().get(i).getCoupon_id());
                        break;
                    case 15:
                        map.put(15, bean.getCouponList().get(i).getSurplus_num());
                        couponId_map.put(6, bean.getCouponList().get(i).getCoupon_id());
                        break;
                    case 20:
                        map.put(20, bean.getCouponList().get(i).getSurplus_num());
        ...
        switch (v.getId()) {
            case R.id.lucky_re:
                if (!SharedData.getInstance().isLogin()) {
                    startActivityForResult(new Intent(LuckyActivityChifeng.this, LoginActivity.class), 23);
                } else {
    ...
    public void showAdvertisingWindow(int index) {
        adv_dialog = new Dialog(LuckyActivityChifeng.this, R.style.ActionSheetDialogStyle);
        View view = LayoutInflater.from(LuckyActivityChifeng.this).inflate(R.layout.lucky_pop, null);
        ...
        switch (index) {
            case 2:
                result_iv.setBackgroundResource(R.mipmap.discount_five);
                break;
            case 4:
                result_iv.setBackgroundResource(R.mipmap.discount_ten);
                break;
            case 6:
                result_iv.setBackgroundResource(R.mipmap.discount_fifteen);
                break;
            case 8:
                result_iv.setBackgroundResource(R.mipmap.discount_twenty);
                break;
            default:
